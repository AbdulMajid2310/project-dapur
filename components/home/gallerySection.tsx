...
export default function GallerySection() {
  ...
  if (loading) return <div className="py-20 text-center text-orange-200">Memuat galeri...</div>;
    ...
    >
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <motion.div className="text-center mb-16" variants={itemVariants}>
          <h2 className="text-3xl md:text-5xl font-extrabold text-white mb-4">{sectionData.title}</h2>
          <div className="w-24 h-1 bg-orange-400 mx-auto rounded-full mb-6"></div>
          <p className="text-orange-100/70 text-lg max-w-2xl mx-auto">{sectionData.description}</p>
        </motion.div>

        {/* Gallery Grid */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 lg:gap-6">
          {galleryData.filter(item => item.isActive).map(item => (
            <motion.div
              key={item.galleryId}
              className="group relative overflow-hidden rounded-3xl shadow-2xl border border-white/10 cursor-pointer"
              variants={itemVariants}
              whileHover={{ y: -10 }}
              onClick={() => setSelectedId(selectedId === item.galleryId ? null : item.galleryId)}
            >
              ...
              >
                <div className="text-white transform translate-y-4 group-hover:translate-y-0 transition-transform duration-500">
                  <span className="text-xs font-bold uppercase tracking-widest text-orange-300 mb-2 block">{item.category}</span>
                  <h3 className="text-xl font-bold mb-1">{item.title}</h3>
                  <p className="text-sm text-orange-100/80 mb-3 line-clamp-2">{item.caption}</p>
                ...
                {selectedId === item.galleryId && (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <span className="text-xs font-bold uppercase tracking-widest text-orange-300">{item.category}</span>
                      <button onClick={() => setSelectedId(null)}>
                        <FaWindowClose className="w-5 h-5 text-orange-200 hover:text-white" />
                      </button>
                    </div>
                    <div className="mt-4 max-h-full overflow-auto h-full scrollbar-hide">
                      <h3 className="text-2xl font-bold mb-2">{item.title}</h3>
                      <p className="text-sm text-orange-100/80 leading-relaxed">{item.caption}</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
        ...
        <motion.div className="mt-20 text-center" variants={itemVariants}>
          <p className="text-orange-100/60 mb-6 italic">"{sectionData.quote}"</p>
          <motion.button
